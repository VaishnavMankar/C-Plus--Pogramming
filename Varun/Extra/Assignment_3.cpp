...
void Quicksort(vector<int> &A, int start, int end, int pivot)
{
	if(start < end)
	{
		int part = partition(A, start, end, pivot);
		Quicksort(A, start, part - 1, pivot);
		Quicksort(A, part + 1, end, pivot);
	}
}
...
int main(int argc, char* argv[])
{
	...
	vector<int> arr(blockSize);

	MPI_Scatter(A.data(), blockSize, MPI_INT, arr.data(), blockSize, MPI_INT, 0, MPI_COMM_WORLD);

	Quicksort(arr, 0, blockSize - 1, pivot);
	cout << endl;
...
void parallel_quicksort(vector<int> &B, MPI_Comm comm)
{
	...
	if(rank < (size / 2))
	{
		...
		int recvSize;
		MPI_Recv(&recvSize, 1, MPI_INT, rank, 0, processorSwapPartner, &status);
...
int main(int argc, char* argv[])
{
	...
	parallel_quicksort(arr, MPI_COMM_WORLD);
	...
	MPI_Gather(&localSize, 1, MPI_INT, recvCounts.data());
