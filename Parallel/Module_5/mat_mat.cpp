...
int main(int argc, char *argv[]){
    ...
    if(rank == 0){
        ...
        matrix_A = { 4, 3, 2, 1, 5, 3, 4, 5, 2, 1, 3, 2, 1, 4, 6, 8 };
        matrix_B = { 2, 3, 5, 8, 8, 7, 3, 6, 6, 4, 1, 7, 9, 1, 9, 3 };

        // Resize the final result matrix to be able to receive the data
        final_C_matrix.resize(N*N);

        cout << "This is our first matrix" << endl; 
        for(int i = 0; i < N; i++){
            for(int j = 0; j < N; j++){
                cout << matrix_A[i*N + j] << " ";
            }
        ...
        cout << "This is out second matrix" << endl;
        for(int i = 0; i < N; i++){
            for(int j = 0; j < N; j++){
                cout << matrix_B[i*N + j] << " ";
            }
    ...
    MPI_Scatter(matrix_A.data(), N, MPI_INT, local_A_row.data(), N, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Gather(local_C_row.data(), N, MPI_INT, final_C_matrix.data(), N, MPI_INT, 0, MPI_COMM_WORLD);

    // Step 5: The master process prints the final result
    if(rank == 0){
        cout << "Final result of both the matrix" << endl;
        for(int i = 0; i < N; i++){
            for(int j = 0; j < N; j++){
